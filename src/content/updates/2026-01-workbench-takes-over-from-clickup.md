...
Three days later the agents got their first real tools: they could read a client website's page inventory and update it, setting a page's type, its anchor keyword and whether we monitor it. That sounds small. It is the difference between an agent that talks about a plan and one that can act on the record.

Underneath the agents, the plan itself changed shape. A plan item, the unit of work a partner approves, now turned into a task with steps on its own, and finishing the task marked the plan item done without anyone touching it. Bulk planning let a manager lay out several months of items for a campaign at once. Locations became a first-class part of a campaign, which multi-location clients had needed for a long time. And a discovery, the audit we run when an agency brings us a prospect, started running as a sequence of automated steps rather than a checklist someone remembered.

None of this was polished. Most of the commits that fortnight are called "quick push". But by the 18th a campaign lived in one place, its history came with it, and three agents could read it.
