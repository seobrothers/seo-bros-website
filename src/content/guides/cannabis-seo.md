...
For the on-page mechanics behind these, see [our on-page SEO guide](/guides/on-page-seo/).
...
For the full local playbook, see [our local SEO guide](/guides/local-seo/).
...
1. **Age gates that block Googlebot.** A poorly built age verification overlay can hide the entire site from search engines. Test with Google's URL Inspection tool.
2. **Boilerplate strain pages copied from POS data.** If every strain page is the same template with the strain name swapped, none rank.
3. **Medical claims that trip YMYL filters.** "CBD cures anxiety" is both non-compliant and a ranking liability. Hedged language, citations, and conservative framing perform better, and the difficulty numbers on condition terms show why credibility is the deciding factor.
4. **Ignoring Weedmaps and Leafly.** With hundreds of thousands of monthly searches between them, these are not optional channels. Operators who treat them as afterthoughts lose to operators who treat them as primary surfaces.
5. **Slow sites loaded with high-resolution product imagery.** Core Web Vitals are a ranking factor. Compress, lazy-load, and serve responsive image sizes.
6. **No location pages for multi-store operators.** Every retail location needs its own page. The home page can't rank for every city.
7. **Thin About and team pages.** YMYL categories reward author and operator credibility. Real bios, real photos, real licenses, real history.
8. **Publishing "cannabis is legal now" content after the 2026 rescheduling.** Recreational cannabis is still federally Schedule I. Overstating the change is an accuracy problem Google increasingly notices.
...
If you've got a dispensary, CBD brand, or cannabis-service client and you're trying to figure out where the leverage actually is, [run a free discovery with us](/tideworthy-sign-up/) and we'll diagnose what's holding the rankings back, then deliver the fix under your brand.
